package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1380:String = "%";
      ...
      public function HabboLocalizationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.events.addEventListener(com.sulake.core.localization.enum.LocalizationEvent.const_89,onLocalizationLoaded);
      }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1380);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1380,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      private function onConfigurationReady(param1:Event = null) : void
      {
         ...
         if("false" == var_168.getKey("use.default.localizations","false"))
         {
            _loc4_ = var_168.getKey("external.texts.txt","external_texts.txt");
            super.loadLocalizationFromURL(_loc4_);
         }
         var _loc3_:int = 1;
         while(var_168.keyExists("localization." + _loc3_))
         {
            _loc5_ = var_168.getKey("localization." + _loc3_,"");
            _loc6_ = var_168.getKey("localization." + _loc3_ + ".name","");
            _loc7_ = var_168.getKey("localization." + _loc3_ + ".url","");
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         if(param2 != null)
         {
            ...
            (var_168 as Component).events.addEventListener(HabboConfigurationEvent.INIT,onConfigurationReady);
         }
      }
      ...
      {
         events.dispatchEvent(new Event(com.sulake.habbo.localization.enum.LocalizationEvent.const_89));
      }
   }
}
