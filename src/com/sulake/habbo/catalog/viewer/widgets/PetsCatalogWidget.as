package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_425 >= var_305.length)
         {
            return "";
         }
         var petColor:IPetColor = var_305[var_425] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_594[var_1560] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_425 = param1.index;
         if(var_425 < 0 || var_425 > var_305.length)
         {
            var_425 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_561.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_145,onTextWindowEvent);
         var_561 = new Map();
         ...
         var_899 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_305 = [];
         var_594 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_899);
               if(_loc11_)
               {
                  var_594 = _loc11_.breeds;
                  var_305 = _loc11_.colors;
                  if(var_305 == null)
                  {
                     var_305 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_594.length)
         {
            _loc12_ = var_594[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_899,_loc12_.id),getRaceLocalizationKey(var_899,_loc12_.id)));
            _loc6_++;
         }
         var_561.add(var_899,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_810,onApproveNameResult);
         _loc5_ = var_561.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_305)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_561.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_425 >= 0 && var_425 < var_305.length)
         {
            _loc7_ = var_305[var_425] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_594[var_1560] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_899,_loc3_.id,_loc2_,AvatarScaleType.const_50);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_295,AvatarAction.const_598);
